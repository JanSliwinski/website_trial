"""Build the v2 diagnostic chart for the production ML pipeline."""
...
from helleniflex import (
    ...
    load_entsoe_flows_directory, load_entsoe_flows_by_neighbor_directory,
...
prices = load_entsoe_prices_directory(ENT); load = load_entsoe_load_directory(ENT)
...
flows = load_entsoe_flows_directory(ENT); flows_pn = load_entsoe_flows_by_neighbor_directory(ENT)
foreign = load_foreign_prices_directory(FOR)

builder = FeatureBuilder(prices=prices, load=load, wind=wind, solar=solar,
                         flows=flows, flows_per_neighbor=flows_pn,
                         gen_total=gen_total, external_prices=foreign)
df = builder.build()
...
if to_drop: train = train.drop(columns=to_drop); test = test.drop(columns=to_drop)

ridge = RidgeMLForecaster(alpha=1.0); ridge.fit(train)
gbm = GBMForecaster(n_estimators=300, learning_rate=0.05, num_leaves=31, min_child_samples=20); gbm.fit(train)

y_actual = test['dam_price_eur_mwh'].values
y_ridge = ridge.predict(test)
y_gbm = gbm.predict(test)

bt = pd.read_csv(os.path.join(ROOT, "data", "revenue_backtest_v2_april_2026.csv"),
...
fig.suptitle(
    "HelleniFlex v2 — Real-Data ML Forecasting Pipeline   ·   "
    "33 features · 5 neighbor markets · Trained 2025, Tested April 2026",
...
ax.plot(hours, y_ridge[:n_show], color='#3a7bd5', lw=1.2, alpha=0.9,
        label=f'Ridge MAE €{np.abs(y_ridge - y_actual).mean():.1f}')
ax.plot(hours, y_gbm[:n_show], color='#e8744f', lw=1.2, alpha=0.9,
        label=f'LightGBM MAE €{np.abs(y_gbm - y_actual).mean():.1f}')
...
ax.set_xlabel('Actual price [€/MWh]'); ax.set_ylabel('Predicted price [€/MWh]')
ax.set_title("Ridge: predicted vs actual", loc='left', fontweight='bold', fontsize=11)
ax.axhline(0, color='gray', lw=0.5); ax.axvline(0, color='gray', lw=0.5); ax.grid(alpha=0.3)
mae_r = np.abs(y_ridge - y_actual).mean()
neg_r = ((y_ridge < 0) & (y_actual < 0)).sum() / max(1, (y_actual < 0).sum())
ax.text(0.04, 0.96, f'MAE €{mae_r:.1f}/MWh\nNeg-recall {neg_r*100:.0f}%',
        transform=ax.transAxes, va='top', fontsize=9,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#888', alpha=0.9))

# Row 2 col 1: LGBM scatter
...
neg_g = ((y_gbm < 0) & (y_actual < 0)).sum() / max(1, (y_actual < 0).sum())
ax.text(0.04, 0.96, f'MAE €{mae_g:.1f}/MWh\nNeg-recall {neg_g*100:.0f}%',
        transform=ax.transAxes, va='top', fontsize=9,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#888', alpha=0.9))

# Row 2 col 2: top features
ax = fig.add_subplot(gs[1, 2])
fi = gbm.feature_importance().head(10)[::-1]
y_pos = np.arange(len(fi))
colors = []
for name in fi.index:
    if 'price_' in name and 'lag' in name and any(cc in name for cc in ['bg','ro','it','mk']):
        colors.append('#d4a017')  # foreign price (highlight)
    elif 'flow_' in name:
        colors.append('#7ed957')  # flow
    else:
        colors.append('#e8744f')  # other
ax.barh(y_pos, fi.values / 1e6, color=colors, alpha=0.85)
ax.set_yticks(y_pos)
ax.set_yticklabels([n.replace('_mw','').replace('_',' ') for n in fi.index], fontsize=8)
ax.set_xlabel('Gain importance (millions)', fontsize=9)
ax.set_title("LightGBM top 10 features\n(yellow=foreign price, green=flow)",
             loc='left', fontweight='bold', fontsize=11)
ax.grid(alpha=0.3, axis='x')

# Bottom: per-day revenue
...
ax.set_xticks(days); ax.set_xticklabels(day_labels, fontsize=8)
ax.set_xlabel('Day of April 2026', fontsize=10); ax.set_ylabel('Realized revenue [€]', fontsize=10)
ax.set_title(
...
out = os.path.join(ROOT, "docs", "ml_pipeline_v2_diagnostics.png")
