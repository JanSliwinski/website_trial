...
export default function BatteryForm({ onSubmit, loading }: Props) {
  ...
  return (
    <div className="rounded border border-aegean-700 bg-aegean-900 p-5 space-y-5">
      {/* Presets */}
      <div>
      ...
      {/* Date */}
      <div>
        <p className="helios-label mb-2">Optimisation Date</p>
        <div className="relative">
          <Calendar size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-marble-500" />
          <input
            type="date"
            value={date}
            min={DATE_MIN}
            ...
            className="w-full rounded border border-aegean-700 bg-aegean-950 py-2 pl-8 pr-3 text-sm text-marble-100 focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500/30 [color-scheme:dark] transition-colors"
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <NumInput
          label="Power"
          value={battery.power_mw}
          min={0.1} max={500} step={0.1} unit="MW"
          onChange={(v) => update("power_mw", v)}
        />
        <NumInput
          label="Capacity"
          value={battery.capacity_mwh}
          min={0.1} max={2000} step={0.1} unit="MWh"
        ...
        <NumInput
          ...
          onChange={(v) => {
            const η = Math.sqrt(v / 100);
            update("eta_charge", η);
            update("eta_discharge", η);
          }}
        />
        <NumInput
          label="Degradation"
          value={battery.cycle_cost_eur_per_mwh}
          min={0} max={30} step={0.5} unit="€/MWh"
      ...
      <button
        ...
        disabled={loading}
        className={cn(
          "w-full rounded py-2.5 text-sm font-bold tracking-[0.08em] uppercase transition-all",
          loading
